...
int varfit( FILE *out_file, arfit_file_info *info, int pmin, int pmax, int zero, arfit_selector selector, arfit_algorithm algorithm, arfit_mode mode, long threshold )
{
	...
	for( VFirstAttr( info->vAttributes, &iterator ); VAttrExists( &iterator ); VNextAttr( &iterator ) ) if( VGetAttrRepn( &iterator ) == VImageRepn ) {
			VGetAttrValue( &iterator, NULL, VImageRepn, &original );

			if( VPixelRepn( original ) == VUByteRepn && VImageNRows( original ) > info->nrows ) VAppendAttr( out_list, "image", NULL, VImageRepn, original );
		}
	...
	for( sl = 0; sl < info->nfunctionalslices; ++sl ) for( row = 0; row < info->nrows; ++row ) for( column = 0; column < info->ncolumns; ++column ) {
				var = 0.0;
