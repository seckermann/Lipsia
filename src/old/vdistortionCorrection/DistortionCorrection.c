...
VDistortionCorrection(VAttrList list, VAttrList list_shift, VDouble sigma, VDouble edist) {
    ...
    for(VFirstAttr(list_shift, & posn); VAttrExists(& posn); VNextAttr(& posn)) {
        ...
        if(VPixelRepn(shift_image) != VFloatRepn)
            continue;
        break;
    }
    if(shift_image == NULL)
        VError(" no shift image found");
    ...
    */
    ...
    for(VFirstAttr(list, & posn); VAttrExists(& posn); VNextAttr(& posn)) {
        ...
        if(VPixelRepn(src) != VUByteRepn)
            continue;
        if(VImageNBands(shift_image) != VImageNBands(src))
            continue;
        if(VImageNRows(shift_image) != VImageNRows(src))
            continue;
        if(VImageNColumns(shift_image) != VImageNColumns(src))
            continue;
    ...
    if(i > 0)
        fprintf(stderr, " processed %d structural images\n", i);
    /*
