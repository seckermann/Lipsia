...
VAlphaSim(VImage src, VImage dest, VLong seed, VLong numiter,
    ...
    */
    if(strlen(filename) > 2) {
        fp = fopen(filename, "w");
        if(!fp)
            VError(" error opening report file '%s' ", filename);
    }
    pthr = z2p(z0);
    fprintf(stderr, "\n");
    ...
    if(VGetAttr(VImageAttrList(src), "voxel", NULL,
        ...
        if(fp)
            fprintf(fp, "  voxel size:  %.2f x %.2f x %.2f = %.2f mm^3\n", x, y, z, voxsize);
    }
    sigma = fwhm / 2.35482;
    sigma /= x;
    fprintf(stderr, "\n");
    ...
    for(i = 0; i < tabsize; i++)
        histogram[i] = table[i] = maxhist[i] = 0;
