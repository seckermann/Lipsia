...
VFDR( VImage src, VShort sign, VFloat alpha )
{
	...
	if ( VGetAttr ( VImageAttrList ( src ), "modality", NULL,
		...
		if ( strcmp( buf, "tmap" ) == 0 ) tmap = 1;

		if ( strcmp( buf, "zmap" ) == 0 ) tmap = 2;

		if ( tmap < 1 ) VError( " image must be 'tmap' or 'zmap', not %s", buf );
	} else
	...
	*/
	if ( tmap == 1 ) {
		...
		for ( i = 0; i < VImageNPixels( src ); i++ ) {
			...
			if ( ABS( t ) < tiny ) continue;

			if ( sign > 0 && t < 0 ) p = 1;
			else if ( sign < 0 && t > 0 ) p = 1;
			else p = t2p( t, df );
	...
	*/
	...
	for ( i = 0; i < VImageNPixels( src ); i++ ) {
		...
		*bin_pp = 0;
		...
		if ( ABS( z ) > 0 ) {
			if ( tmap == 1 ) p = t2p( z, df );
			else p = z2p( z );

			if ( p <= p0 ) {
				if ( ( sign > 0 && z > 0 ) || ( sign < 0 && z < 0 ) ) *bin_pp = 1;
			}
		}
	...
	if ( msize > 0 ) {
		...
		if ( nl < 1 ) VWarning( " no voxels found, perhaps areas too small" );
