...
VImageSum( VImage src, VImage dest, VFloat pos, VFloat neg )
{
	...
	for ( i = 0; i < VImageNPixels( src ); i++ ) {
		if ( ( *src_pp >= pos ) || ( *src_pp <= neg ) ) *dest_pp += 1;
...
Usage:
		...
		for ( VFirstAttr ( list, & posn ); VAttrExists ( & posn ); VNextAttr ( & posn ) ) {
			...
			if ( dest == NULL ) {
				...
				if ( type == 1 ) {
					...
					if ( nimages >= VPixelMaxValue( count ) ) VError( " too many input images" );
				}
			}
	...
	if ( dest == NULL ) VError( " input image not found" );
	...
	if ( type == 1 ) {
		...
		for ( i = 0; i < VImageNPixels( dest ); i++ ) {
			if ( *cnt_pp < 0 ) *dest_pp = -( *dest_pp );

			if ( ABS( *cnt_pp ) < nimages ) *dest_pp = 0;
