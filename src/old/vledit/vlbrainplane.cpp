...
void vlBrainPlane::createTexture( bool newImage )
{
	...
	if ( anaPixel != NULL ) {
		if ( DATAMANAGER->isValid() ) {
			...
			switch( VPixelRepn( DATAMANAGER->image()->src() ) ) {
			case VUByteRepn:
				delete[] ( VUByte * )anaPixel;
				break;
			case VSByteRepn:
				delete[] ( VSByte * )anaPixel;
				break;
			case VShortRepn:
				delete[] ( VShort * )anaPixel;
				break;
			default:
				qWarning( "Datentyp nicht zulaessig" );
				abort();
...
void vlBrainPlane::deleteVoxel( int x, int y, int radius )
{
	...
	switch ( m_type ) {
	case CORONAL:
		DATAMANAGER->deleteVoxel( sel, y, m_anaPlanePos, x, radius );
		break;
	case SAGITTAL:
		DATAMANAGER->deleteVoxel( sel, y, x, m_anaPlanePos, radius );
		break;
	case AXIAL:
		DATAMANAGER->deleteVoxel( sel, m_anaPlanePos, y, x, radius );
		break;
	}
...
void vlBrainPlane::deleteSegment( int x, int y, int radius, int delta )
{
	...
	switch ( m_type ) {
	case CORONAL:
		DATAMANAGER->deleteSegment( sel, y, m_anaPlanePos, x, radius, delta );
		break;
	case SAGITTAL:
		DATAMANAGER->deleteSegment( sel, y, x, m_anaPlanePos, radius, delta );
		break;
	case AXIAL:
		DATAMANAGER->deleteSegment( sel, m_anaPlanePos, y, x, radius, delta );
		break;
	}
...
void vlBrainPlane::markSegment( int x, int y )
{
	...
	switch ( m_type ) {
	case CORONAL:
		DATAMANAGER->selectSegment( y, m_anaPlanePos, x, UICONFIG->radius()
									, UICONFIG->delta() );
		break;
	case SAGITTAL:
		DATAMANAGER->selectSegment( y, x, m_anaPlanePos, UICONFIG->radius()
									, UICONFIG->delta() );
		break;
	case AXIAL:
		DATAMANAGER->selectSegment( m_anaPlanePos, y, x, UICONFIG->radius()
									, UICONFIG->delta() );
		break;
	}
...
void vlBrainPlane::markVoxel( int x, int y )
{
	...
	switch ( m_type ) {
	case CORONAL:
		DATAMANAGER->selectVoxel( y, m_anaPlanePos, x, UICONFIG->radius() );
		break;
	case SAGITTAL:
		DATAMANAGER->selectVoxel( y, x, m_anaPlanePos, UICONFIG->radius() );
		break;
	case AXIAL:
		DATAMANAGER->selectVoxel( m_anaPlanePos, y, x, UICONFIG->radius() );
		break;
	}
