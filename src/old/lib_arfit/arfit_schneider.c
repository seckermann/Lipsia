...
arfit_output *arfit_schneider(arfit_input *input, arfit_output **output) {
    ...
    if(nopt > 0) {
        if(input->zero == 1) {
            ...
            for(i = 2; i < scale->size; ++i)
                if(value < gsl_vector_get(scale, i))
        ...
        if(input->zero == 1) {
            ...
            for(i = 0; i < Aaug->size1; ++i)
                for(j = 1; j < Aaug->size2; ++j) {
    ...
    if(nopt > 0) {
        ...
        for(i = 1; i < R11->size1; ++i)
            for(j = 0; j < R11->size2; ++j)
