...
VBoolean VGaussianKernel(int ncoeffs, VDouble coeffs[], double s) {
    ...
    /* Setup depends on whether the number of coefficients asked for is
       odd or even: */
    if(ncoeffs & 1) {
        p1 = & coeffs[ncoeffs / 2];
        *p1-- = 1.0;
...
VImage VGaussianConv(VImage src, VImage dest, VBand band,
    ...
    /* Convert the source image to Float or Double for the sake of
       efficiency: */
