...
main(int argc, char *argv[]) {
    ...
    for(VFirstAttr(list, & posn); VAttrExists(& posn); VNextAttr(& posn)) {
        ...
        i++;
        src[i] = xsrc;
        if(VImageNRows(xsrc) < 2)
            continue;
        if(VImageNBands(xsrc) > len)
            len = VImageNBands(xsrc);
    ...
    */
    ...
    if(mask == NULL)
        VError(" specify either mask or voxel addr");
    /*
    ** get max-id
    */
    ...
    */
    if(strlen(rep_filename) > 1) {
        ...
        if(!fpe)
            VError(" err opening report file %s", rep_filename);
        for(i = 0; i < len; i++) {
