...
main ( int argc, char *argv[] )
{
	...
	for ( VFirstAttr ( in_list, & posn ); VAttrExists ( & posn ); VNextAttr ( & posn ) ) {
		...
		if ( VPixelRepn( src ) == VShortRepn ) {
			...
			if ( bands < 2 && VImageNBands( src ) > 1 ) bands = ( int )VImageNBands( src );
			else {
				if ( bands > 1 && VImageNBands( src ) > 1 && bands != VImageNBands( src ) )
					VError( "Different bands in images" );
			}

			if ( rows < 2 && VImageNRows( src ) > 1 ) rows  = ( int )VImageNRows( src );
			else {
				if ( rows > 1 && VImageNRows( src ) > 1 && rows != VImageNRows( src ) )
					VError( "Different rows in images" );
			}

			if ( cols < 2 && VImageNColumns( src ) > 1 ) cols  = ( int )VImageNColumns( src );
			else {
	...
	/* Check input data */
	if ( bands < 2 || rows < 2 || cols < 2 ) VError( "Check image dimensions and data" );

	if ( i == 0 ) VError( "No functional slices" );
	else fprintf( stderr, "Functional slices: %d, Timespeps: %d\n", i, bands );
	...
	if ( choice == 1 ) {
		...
		*/
		...
		if ( xslice  >= i )       VError( "Choose a slice between 0 and %d", i - 1 );

		if ( xrow    >= rows )    VError( "Choose a row   between 0 and %d", rows - 1 );

		if ( xcolumn >= cols )    VError( "Choose a col   between 0 and %d", cols - 1 );

		if ( strlen( filename ) > 2 ) VError( "Do not use a reference-file with '-choice voxel'" );
		...
		if ( strlen( zmapfile ) > 3 ) {
			...
			if ( ! listz ) VError( "Error reading zmap" );
			...
			for ( VFirstAttr ( listz, & posn ); VAttrExists ( & posn ); VNextAttr ( & posn ) ) {
				...
				if ( VPixelRepn( zmap ) != VFloatRepn ) continue;
			...
			if ( VPixelRepn( zmap ) != VFloatRepn ) VError( " zmap not found" );
		...
		if ( time == NULL ) time = ( double * ) malloc( sizeof( double ) * bands );

		for ( j = 0; j < bands; j++ ) time[j] = ( double )0;
		...
		/* divide */
		if ( nl == 0 ) VError( "No pixel found in mask" );
		else fprintf( stderr, "mask contains %d voxels\n", nl );

		for ( j = 0; j < bands; j++ ) time[j] /= ( double )nl;
	...
	if ( choice != 1 ) {
		...
		if ( choice == 2 ) {
			for ( j = 0; j < bands; j++ ) time[j] = sin( ( double )j * piii / freqsec );
		}

		if ( choice == 3 ) {
			for ( j = 0; j < bands; j++ ) time[j] = -ABS( sin( ( double )j * piii / freqsec / 2 ) );
		}

		/* Reference file used */
		if ( choice == 4 ) {
			if ( strlen( filename ) < 3 ) VError( "A reference-file has to be specified with '-choice file'" );

			fp2 = fopen( filename, "r" );

			if ( fp2 == NULL ) VError( " err opening %s", filename );
			...
			while ( fgets( buf, 128, fp2 ) ) {
				...
				if ( nn >= bands ) VError( "The length of the reference-timecourse has to be %d\n", bands );
			...
			if ( nn != bands ) VError( "The length of the reference-timecourse must be %d\n", bands );
		} else {
			if ( strlen( filename ) > 2 ) VError( "Specify a reference file for '-choice file' only" );
		}
	}


	/* mean of reference signal */
	for ( j = 0; j < bands; j++ ) mean += time[j];

	mean /= ( double )bands;

	/* covariance sequence of reference signal */
	if ( swd == -1 ) swd = ( int )( bands / 10 );

	if ( cov2 == NULL ) cov2 = ( double * ) malloc( sizeof( double ) * ( swd + 1 ) );
	...
	if ( strlen( report ) > 2 ) {
		...
		for ( j = 0; j < bands; j++ ) fprintf( fp1, "%f\n", time[j] );
	...
	for ( t = -swd; t <= swd; t++ ) edf += klag( ( double )t / ( double )swd, ( double )lag );
	...
	if ( edf < 20 || nu < 10 ) VWarning( "Effective degrees of freedom too small" );
	...
	if ( voxel ) VSetAttr( VImageAttrList ( dest ), "voxel", NULL, VStringRepn, voxel );

	if ( extent ) VSetAttr( VImageAttrList ( dest ), "extent", NULL, VStringRepn, extent );

	if ( orientation ) VSetAttr( VImageAttrList ( dest ), "orientation", NULL, VStringRepn, orientation );
	...
	if ( voxel ) VSetAttr( VImageAttrList ( destCI ), "voxel", NULL, VStringRepn, voxel );

	if ( extent ) VSetAttr( VImageAttrList ( destCI ), "extent", NULL, VStringRepn, extent );

	if ( orientation ) VSetAttr( VImageAttrList ( destCI ), "orientation", NULL, VStringRepn, orientation );
	...
	if ( voxel ) VSetAttr( VImageAttrList ( phase ), "voxel", NULL, VStringRepn, voxel );

	if ( extent ) VSetAttr( VImageAttrList ( phase ), "extent", NULL, VStringRepn, extent );

	if ( orientation ) VSetAttr( VImageAttrList ( phase ), "orientation", NULL, VStringRepn, orientation );
	...
	if ( voxel ) VSetAttr( VImageAttrList ( phaseCI ), "voxel", NULL, VStringRepn, voxel );

	if ( extent ) VSetAttr( VImageAttrList ( phaseCI ), "extent", NULL, VStringRepn, extent );

	if ( orientation ) VSetAttr( VImageAttrList ( phaseCI ), "orientation", NULL, VStringRepn, orientation );

	VAppendAttr( out_list, "image", NULL, VImageRepn, phaseCI );

	/* Append mask image */
	if ( mask ) VAppendAttr( out_list, "image", NULL, VImageRepn, mask );
