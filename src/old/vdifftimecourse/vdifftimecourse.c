...
int main ( int argc, char *argv[] )
{
	...
	if ( ! first_list ) VError( "Error reading first file" );
	...
	if ( ! second_list ) VError( "Error reading second file" );
	...
	for ( VFirstAttr ( first_list, & fposn ); VAttrExists( & fposn ); VNextAttr( & fposn ) ) {
		if ( VGetAttrRepn ( & fposn ) != VImageRepn ) continue;
	...
	*/
	...
	for ( VFirstAttr ( second_list, & sposn ); VAttrExists( & sposn ); VNextAttr( & sposn ) ) {
		if ( VGetAttrRepn ( & sposn ) != VImageRepn ) continue;

		VGetAttrValue ( & sposn, NULL, VImageRepn, & ssrc );

		if ( VPixelRepn( ssrc ) == VUByteRepn ) continue;
	...
	for ( VFirstAttr ( second_list, & sposn ); VAttrExists ( & sposn ); VNextAttr ( & sposn ) ) {
		...
		if ( VPixelRepn( ssrc ) == VShortRepn ) break;
	}
	...
	for ( VFirstAttr ( first_list, & fposn ); VAttrExists ( & fposn ); VNextAttr ( & fposn ) ) {
		...
		if ( VPixelRepn( fsrc ) != VShortRepn ) continue;
		...
		if ( ( int )VImageNRows( fsrc ) < 2 ) goto next1;
