...
my_f (const gsl_vector *vec, void *params)
{
  ...
  for (b=b0; b<nslices-b0; b++) {
	...
	if (cp > -0.01 && cp < 0) cp = 0;
	if (rp > -0.01 && rp < 0) rp = 0;
	if (bp > -0.01 && bp < 0) bp = 0;
	...
	/* check subcube */
        if (rr < 0 || rr >= nrows-1) continue;
        if (cc < 0 || cc >= ncols-1) continue;
	if (bb < 0 || bb >= nslices) continue;
	if (VImageNRows(src[bb]) < 2) continue;
	...
	if (wsum < 0.999 && wsum > 0) v += (1.0 - wsum)*v;
...
VMotionCorrection3d(VAttrList list,VLong i0,VLong step,VLong maxiter)
{
  ...
  */
  ...
  for (i=0; i<n; i++) {
    if (i%10 == 0) fprintf(stderr,"#   %6d  of %6d\r",i,n);
