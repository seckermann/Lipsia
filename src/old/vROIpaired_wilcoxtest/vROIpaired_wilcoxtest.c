	...
	for ( id = 1; id <= nROI; id++ ) {
		...
		for ( i = 0; i < n; i++ ) {
			...
			for ( vol = volumes[j]->first; vol != NULL; vol = vol->next ) {
				...
				for ( j = 0; j < VolumeNBuckets( vol ); j++ ) {
					for ( tc = VFirstTrack( vol, j ); VTrackExists( tc ); tc = VNextTrack( tc ) ) {
						...
						for ( c = c0; c < c1; c++ ) {
							...
							if ( ABS( u ) < tiny || ABS( v ) < tiny ) continue;
				...
				if ( nx < 1 ) VError( " no voxels in ROI %d, image %d", id, i + 1 );
...
int main ( int argc, char *argv[] )
{
	...
	if ( in_files2.number != nimages ) VError( " inconsistent number of images: %d %d ",
	...
	for ( i = 0; i < nimages; i++ ) {
		...
		if ( ! list1 )  VError( "Error reading file %s", in_filename );
	...
	for ( i = 0; i < nimages; i++ ) {
		...
		if ( ! list2 )  VError( "Error reading file %s", in_filename );
