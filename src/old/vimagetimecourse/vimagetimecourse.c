...
int main(int argc, char *argv[]) {
    ...
    if(minval == 0)
        fprintf(stderr, "Determining minval...\n");
    nslice    = 0;
    fullslice = 0;
    grandmean = 0;
        ...
        if(VPixelRepn(src) == VShortRepn) {
            ...
            if(tr == 0) {
                if(VGetAttr(VImageAttrList(src), "MPIL_vista_0", 0, VStringRepn, &str)
                    ...
                    VError("TR in functional data missing");
            }
            nslice++;
            ...
            if(ntimesteps == 0) {
                ...
                if(ntimesteps != VImageNBands(src))
                    VError("Different number of timesteps in slices");
                if(nrows  != VImageNRows(src))
                    VError("Different number of rows in slices");
                if(ncols  != VImageNColumns(src))
                    VError("Different number of columns in slices");
            }
...
next:
    ...
    if(old) {
        ...
        ntrials = j;
        if(ntrials == 0)
            VError("No onsets for condition found");
    } else {
        j = i = 0;
        while(!feof(des_file)) {
        ...
        ntrials = j;
        if(ntrials < 1)
            VError(" no trials in condition %d", cond);
    }
    ntrials--;
    fclose(des_file);
    ...
    for(j = 0; j < ntrials; j++)
        if(old)
            fprintf(rep_file, " %.1f", x[j]);
        else
            fprintf(rep_file, " %.1f", x[j]*tr);
    fprintf(rep_file, "\n\n");
    ...
    for(VFirstAttr(in_list, & posn); VAttrExists(& posn); VNextAttr(& posn)) {
        ...
        for(rr = 0; rr < nrows; rr++) {
            for(cc = 0; cc < ncols; cc++) {
                if(VPixel(src, 0, rr, cc, VShort) < minval)
                    continue;
                for(j = 0; j < len; j++) {
                    sum1 = nx = 0;
                    for(i = 0; i < ntrials; i++) {
                        ...
                        k1 = floor(s / tr);
                        k2 = k1 + 1;
                        if(k1 >= ntimesteps || k2 >= ntimesteps)
                            continue;
                        if(k1 < 0 || k2 < 0)
                            continue;
