 ...
 *
 * $Id: FreqFilter1.c 3181 2008-04-01 15:19:44Z karstenm $
 *
 *****************************************************************/
...
VFreqFilter( VAttrList list, VFloat high, VFloat low, VBoolean stop, VFloat sharp )
{
	...
	fftw_complex *out;
	fftw_plan p1, p2;
	int i, j, k, n, nc, r, c, nrows, ncols, not = 0;
	...
	*/
	...
	for ( VFirstAttr ( list, & posn ); VAttrExists ( & posn ); VNextAttr ( & posn ) ) {
		...
		if ( k % 2 == 0 ) fprintf( stderr, "# preprocess slice %3d\r", k );
		...
		for ( r = 0; r < nrows; r++ ) {
			for ( c = 0; c < ncols; c++ ) {
				...
				for ( j = 0; j < n; j++ ) {
					in[j] = ( double )VPixel( src, j, r, c, VShort );
					sum  += in[j];
				}
				...
				if ( ABS( sum ) > eps ) {
					...
					for ( j = 0; j < n; j++ )
						VPixel( src, j, r, c, VShort ) = ( VShort )in[j];
				} else not++;
			}
		}
	}
