...
main(int argc, char *argv[]) {
    ...
    */
    ...
    if(! list1)
        VError("Error reading multicomp image");
    fclose(fp);
    for(VFirstAttr(list1, & posn); VAttrExists(& posn); VNextAttr(& posn)) {
        if(VGetAttrRepn(& posn) != VImageRepn)
            continue;
        if(strcmp(VGetAttrName(&posn), "multicomp") != 0)
            continue;
        VGetAttrValue(& posn, NULL, VImageRepn, & multicomp);
        if(VPixelRepn(multicomp) != VFloatRepn)
            continue;
        symm = FALSE;
        ...
        if(VGetAttr(VImageAttrList(multicomp), "corrected-p-threshold", NULL,
                    VFloatRepn, (VPointer) & p0) != VAttrFound)
            p0 = 0.05;
        fprintf(stderr, " Using corrected p-threshold: %.4f\n", p0);
        break;
    }
    if(multicomp == NULL)
        VError(" multicomp image not found");
    if(VImageNBands(multicomp) < 2)
        symm = FALSE;
    /*
    ** Read the input file:
    */
