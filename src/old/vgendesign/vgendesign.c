...
/*
** Glover kernel, gamma functio, parameters changed for block designs
*/
double
...
main(int argc, char *argv[]) {
    ...
    static VFloat  block_threshold = 10;
   ...
   char prg_name[100];
    ...
    if(understrength < 0)
        VError("understrength must be >= 0");
    if(delay < 0)
        VError("delay must be >= 0");
    if(deriv < 0 || deriv > 2)
        VError(" parameter 'deriv' must be less than 3");
    /*
    ** constant TR
    */
    if(strlen(filename) < 3) {
        if(tr > 100)
            VWarning(" TR must be given in seconds, not milliseconds");
        if(tr < 0.0001)
            VError(" TR must be specified");
        if(ntimesteps < 2)
            VError(" 'ntimesteps' must be specified");
    ...
    */
        ...
        while(!feof(fp)) {
            ...
            if(! test_ascii((int)buf[0]))
                VError(" scan file must be a text file");
            i++;
        }
        rewind(fp);
        ntimesteps = i;
    ...
    */
    ...
    for(i = 0; i < nevents; i++) {
        ...
        for(j = 0; j < ntrials; j++) {
            if(trial[j].id != i)
                continue;
            if(trial[j].duration < xmin)
                xmin = trial[j].duration;
        }
        block = FALSE;
