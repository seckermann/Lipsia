...
VHistoEqualizer( VImage src, VImage dest, VDouble background )
{
	...
	if ( VPixelRepn( src ) != VUByteRepn ) VError( " input image must be ubyte." );
	...
	for ( j = 0; j < 256; j++ ) histo[j] = lut[j] = 0;
	...
	for ( j = ( int )VRint( ( double )background ); j < 256; j++ ) histo[j] /= sum;


	for ( i = ( int )VRint( ( double )background ); i < 256; i++ ) {
		sum = 0;

		for ( j = ( int )VRint( ( double )background ); j <= i; j++ ) sum += histo[j];
...
VPeel( VImage src, VImage dest, VDouble d1, VDouble d2, VDouble alpha )
{
	...
	for ( b = 0; b < nbands; b++ ) {
		for ( r = 0; r < nrows; r++ ) {
			for ( c = 0; c < ncols; c++ ) {
				...
				if ( b > fix.z + 20 && r < fix.y )    t = 2.0 * d1;

				if ( b > fix.z + 35 && r < fix.y + 25 ) t = 3.0 * d1;

				if ( b > fix.z + 30 && r > fix.y + 40 ) t = 2.0 * d1;

				if ( t < 3.0 && b > fix.z + 35 && r > fix.y + 10 && r < fix.y + 40 && ABS( c - fix.x ) > 15 )
					t = 2.5 * d1;

				if ( b > fix.z + 55 ) t = 4.0 * d1;

				/* temporal base */
				if ( b > fix.z + 30 && b < fix.z + 40 && ABS( c - fix.x ) > 15
					 && r > fix.y - 22 && r < fix.y + 5 ) t = 1.5 * d1;

				u = VPixel( dt_image, b, r, c, VFloat );

				if ( u < t ) VPixel( bin_image, b, r, c, VBit ) = 0;
			}
		}
	}
	...
	for ( b = 0; b < nbands; b++ ) {
		for ( r = r0; r < nrows; r++ ) {
			for ( c = c0; c <= c1; c++ ) {
				if ( b > fix.z + 10 && r < fix.y + 50 ) continue;

				if ( VPixel( mask_image, b, r, c, VBit ) > 0 ) continue;

				if ( VPixel( dt_image, b, r, c, VFloat ) > 2.0 ) continue;
	...
	for ( b = b0; b < nbands; b++ ) {
		for ( r = r0; r < r1; r++ ) {
			for ( c = c0; c <= c1; c++ ) {
				...
				if ( VPixel( dt_image, b, r, c, VFloat ) > 3.0 ) continue;
	...
	for ( b = b0; b < nbands; b++ ) {
		for ( r = 0; r < nrows; r++ ) {
			for ( c = c0; c <= c1; c++ ) {
				if ( VPixel( tmp, b, r, c, VUByte ) < 240 ) continue;
