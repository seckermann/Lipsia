...
VWhiteRegression( ListInfo *linfo, int numsub, VImage design, VShort numlags, VShort type, VShort numcon, VArgVector contrast, VString confile )
{
	...
	for ( k = 0; k < numsub; k++ ) {
		...
		if ( reptime == 0 ) reptime = tr;
		else if ( reptime != tr ) VError( " inconsistent repetition times: %d %d", tr, reptime );

		if ( nslices == 0 ) nslices = n;
		else if ( nslices != n ) VError( " inconsistent image dimensions, slices: %d %d", n, nslices );

		if ( nrows == 0 ) nrows = nr;
		else if ( nrows != nr ) VError( " inconsistent image dimensions, rows: %d %d", nr, nrows );

		if ( ncols == 0 ) ncols = nc;
		else if ( ncols != nc ) VError( " inconsistent image dimensions, cols: %d %d", nc, ncols );
	}
	...
	if ( n >= MBETA )  VError( " too many covariates (%d), max is %d", n, MBETA );

	if ( m != nbands ) VError( " design dimension inconsistency: %d %d", m, nbands );
	...
	*/
	...
	if ( contrast.number > 0 ) {
		...
		if ( !fp ) VError( " error reading contrast file" );
		...
		while ( !feof( fp ) ) {
			...
			if ( j < 1 ) continue;
			else if ( n != j ) VError( " check length of contrast vector no %d", nr + 1 );

			nr++;

			if ( nr > numcon ) VError( " mismatch between numcon parameter and number of contrasts in confile" );
		}

		if ( nr != numcon ) VError( " mismatch between numcon and number of contrasts in confile" );
	...
	*/
	...
	for ( slice = 0; slice < nslices; slice++ ) {
		...
		for ( k = 0; k < numsub; k++ ) {
			if ( linfo[k].zero[slice] == 0 ) slicedata = 0;
		}
		...
		if ( slicedata == 1 ) {
			for ( k = 0; k < numsub; k++ ) {
				...
				if ( fd == -1 ) VError ( "could not open file %s", linfo[k].filename );
