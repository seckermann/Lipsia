...
Usage:
    ...
    /* Check plausibility */
    if(numlags < 1 || numlags > 8)
        VError("Choose numlags between 1 and 8");
    if(numcon < 1 || numcon > NCON)
        VError("Specify correct number of contrasts");
    if(confile == NULL && contrast.number == 0)
        VError("Specify contrast vector or contrast file");
    if(confile != NULL && contrast.number > 0)
        VError("Specify either contrast vector or file");
    str = VMalloc(100);
    str1 = VMalloc(100);
    if(numcon == 1)
        ...
        sprintf(str, "1.v");
        strcpy(str1, (const char *)out_filename);
    ...
    for(VFirstAttr(list1, & posn); VAttrExists(& posn); VNextAttr(& posn)) {
        ...
        if(VPixelRepn(design) != VFloatRepn)
            continue;
        n++;
        break;
    }
    ...
    for(i = 0; i < numcon; i++) {
        ...
        if(!f)
            VError(" error opening out file %s", out_filename);
        if(! VWriteFile(f, out_list[i]))
            exit(1);
    }
    printf("%s: done.\n", argv[0]);
    return 0;
