...
VMulticomp3d(VImage src, VImage dest, VLong seed, VLong numiter,
    ...
    */
    if(fwhm > 20)
        VWarning(" fwhm seems quite large (%.3f mm) ?!", fwhm);
    cmaxsize = zmaxsize = 0;
    msize = MSIZE_3D;
    csize = CSIZE_3D;
    ...
    */
    ...
    for(iter = 0; iter < numiter; iter++) {
        ...
        */
        ...
        if(nl < 1)
            VError(" no voxels above threshold %f", zthr);
        /*
        ** get size, max value and symmetry  per cluster
        */
        for(i = 0; i < tabsize; i++)
            clustersize[i] = clustermax[i] = clustersym[i] = 0;
        ...
        */
        ...
        for(i = 1; i < tabsize; i++) {
            ...
            if(u < s[0])
                ...
                l = 1;     /* somewhat symmetric */
            else if(u >= s[1] && u < s[2])
                l = 2;     /* symmetric */
            else
                l = 3;                               /* strongly symmetric */
            if(l > sysize)
                sysize = l;
