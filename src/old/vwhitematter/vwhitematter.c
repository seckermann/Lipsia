		 ...
		 VLong high, VLong add, VBoolean tauto, VShort maxiter, VBoolean topology )
{
	...
	*/
	if ( low < BACKGROUND ) low = BACKGROUND;
	...
	*/
	...
	for ( greyval = 1; greyval <= high; greyval++ ) {
		...
		for ( b = 0; b < nbands; b++ ) {
			for ( r = 0; r < nrows; r++ ) {
				for ( c = 0; c < ncols; c++ ) {
					if ( VPixel( src, b, r, c, VUByte ) > greyval ) continue;
		...
		while ( ndel > 0 ) {
			...
			for ( j = 1; j < nn; j++ ) {
				...
				if ( b < 1 || r < 1 || c < 1 ) continue;

				if ( b >= nbands - 1 || r >= nrows - 1 || c >= ncols - 1 ) continue;

				/* check if border voxel */
				dir0 = dir1 = dir2 = dir3 = dir4 = dir5 = FALSE;
				flag = 0;

				if ( VPixel( dest, b, r, c - 1, VBit ) == 0 ) { dir2 = TRUE; flag = 1; }

				if ( VPixel( dest, b, r, c + 1, VBit ) == 0 ) { dir3 = TRUE; flag = 1; }

				if ( VPixel( dest, b, r - 1, c, VBit ) == 0 ) { dir0 = TRUE; flag = 1; }

				if ( VPixel( dest, b, r + 1, c, VBit ) == 0 ) { dir1 = TRUE; flag = 1; }

				if ( VPixel( dest, b - 1, r, c, VBit ) == 0 ) { dir4 = TRUE; flag = 1; }

				if ( VPixel( dest, b + 1, r, c, VBit ) == 0 ) { dir5 = TRUE; flag = 1; }
				...
				if ( topology ) {
					if ( VSimplePoint( dest, b, r, c, 26 ) == 0 ) continue;
				}

				/* enforce geometry constraints */
				if ( geometry && VPixel( src, b, r, c, VUByte ) > low ) {
					if ( FinalPoint( dest, b, r, c ) < 2 ) continue;

					if ( VCheckPointDir( dest, b, r, c, dir0, dir1, dir2, dir3, dir4, dir5 ) == 0 ) continue;
				}
		...
		if ( greyval % 25 == 0 ) fprintf( stderr, " %3d: %7d pts deleted\n", greyval, sumdel );
	}
			  ...
			  VDouble threshold, VLong low, VLong high )
{
	...
	if ( d < 25 ) return GREY; /* outmost 2.0 mm are assumed to be cortex */

	u = VPixel( src, b, r, c, VUByte );

	if ( u < low )  return GREY;

	if ( u > high ) return WHITE;
	...
	if ( u <= mean1 ) return GREY;

	if ( mean1 < 1 ) return GREY;
	...
	if ( mean2 / mean1 < threshold ) return GREY;
	else return WHITE;
}
...
FinalPoint( VImage src, int b, int r, int c )
{
	...
	if ( b > 0 )
		if ( VPixel( src, b - 1, r, c, VBit ) > 0 ) n++;

	if ( b < nbands - 1 )
		if ( VPixel( src, b + 1, r, c, VBit ) > 0 ) n++;

	if ( r > 0 )
		if ( VPixel( src, b, r - 1, c, VBit ) > 0 ) n++;

	if ( r < nrows - 1 )
		if ( VPixel( src, b, r + 1, c, VBit ) > 0 ) n++;

	if ( c > 0 )
		if ( VPixel( src, b, r, c - 1, VBit ) > 0 ) n++;
