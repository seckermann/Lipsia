...
ReadDesign(VString designfile) {
    ...
    if(!fp)
        VError(" error opening design file %s", designfile);
    i = ntrials = nevents = 0;
    while(!feof(fp)) {
        for(j = 0; j < LEN; j++)
            buf[j] = '\0';
...
main(int argc, char *argv[]) {
    ...
    */
    ...
    while(token) {
        if(i > MAXCOND)
            VError(" too many conditions");
        cond[i] = atoi(token);
        if(cond[i] > nevents)
            VError(" illegal condition (%d), max is %d", cond[i], nevents);
        if(cond[i] < 1)
            VError(" illegal condition id, must be positive");
        token = strtok(NULL, " ");
        i++;
    }
    numcond = i;
    ...
    */
    ...
    for(VFirstAttr(list, & posn); VAttrExists(& posn); VNextAttr(& posn)) {
        ...
        if(strcmp(str, "X") == 0) {
            ...
            src[i++] = xsrc;
            if(VImageNBands(xsrc) > ntimesteps)
                ntimesteps = VImageNBands(xsrc);
    ...
    */
    ...
    if(c0 < 0 || c0 >= ncols)
        VError("illegal voxel coordinate");
    if(r0 < 0 || r0 >= nrows)
        VError("illegal voxel coordinate");
    if(b0 < 0 || b0 >= nslices)
        VError("illegal voxel coordinate");
    ...
    */
    if(strlen(mask_filename) > 2) {
        ...
        if(adj == 1) {     /* 6-adjacency */
            if(b0 < nslices - 1)
                VPixel(mask, b0 + 1, r0, c0, VBit) = 1;
            if(b0 > 1)
                VPixel(mask, b0 - 1, r0, c0, VBit) = 1;
            if(r0 < nrows - 1)
                VPixel(mask, b0, r0 + 1, c0, VBit) = 1;
            if(r0 > 1)
                VPixel(mask, b0, r0 - 1, c0, VBit) = 1;
            if(c0 < ncols - 1)
                VPixel(mask, b0, r0, c0 + 1, VBit) = 1;
            if(c0 > 1)
                VPixel(mask, b0, r0, c0 - 1, VBit) = 1;
        }
    ...
    */
    ...
    if(vol0 == NULL)
        VError(" no voxels meeting the specifications were found");
    i = VolumeSize(vol0);
    if(i == 0)
        VError(" no voxels in ROI");
    if(i > 1000)
        VWarning(" more than 1000 voxels in ROI (%d voxels) !", i);
    /*
    ...
    */
    if(strlen(rep_filename) > 2) {
        ...
        if(!fpe)
            VError(" error opening report file %s", rep_filename);
    } else
        fpe = stderr;
    fprintf(fpe, "#\n");
    ...
    */
    for(id = 0; id < numcond; id++) {
        ...
        nx = 0;
        for(j = 0; j < ntrials; j++) {
            if(trial[j].id != cond[id] - 1)
                continue;
            if(trial[j].onset + length >= experiment_duration)
                continue;
            nx++;
            k = 0;
