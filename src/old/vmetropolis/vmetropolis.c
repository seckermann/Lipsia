...
main ( int argc, char *argv[] )
{
	...
	if ( ntypes >= M ) VError( " max number(%d) of event types exceeded", M );
	...
	*/
	...
	if ( ntrials >= N ) VError( " max number(%d) of trials exceeded", N );
	...
	if ( !fp_out ) VError( " error opening report file %s", out_filename );
	...
	for ( i = 0; i < ntypes; i++ ) {
		...
		if ( sum < 0.0001 && nx == 0 ) continue;

		if ( nx == ntypes ) {
			for ( j = 0; j < ntypes; j++ ) mat[i][j] = -1;
	...
	for ( i = 0; i < ntypes; i++ ) {
		for ( j = 0; j < histo[i]; j++ ) trial_tmp[k++] = i;
	}
...
nochmal:
		...
		for ( k = 0; k < i; k++ ) if ( list[k] == j ) goto nochmal;
	...
	for ( i = 0; i < ntypes; i++ ) {
		...
		for ( j = 0; j < ntypes; j++ ) sum += cooc[i][j];

		for ( j = 0; j < ntypes; j++ ) cooc[i][j] /= sum;
	}
