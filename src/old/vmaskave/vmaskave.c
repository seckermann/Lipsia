...
Usage:
	...
	if ( ! list )  VError( "Error reading mask image" );
	...
	for ( VFirstAttr ( list, & posn ); VAttrExists ( & posn ); VNextAttr ( & posn ) ) {
		...
		if ( VPixelRepn( mask ) != VUByteRepn ) VError( " mask image must be of type ubyte" );

		break;
	}

	if ( mask == NULL ) VError( "mask image not found" );
	...
	*/
	...
	if ( strlen( report_filename ) > 1 ) {
		...
		if ( fp == 0 ) VError( " error opening report file %s", report_filename );
	} else
		fp = stderr;
	...
	*/
		...
		for ( i = 0; i < in_files.number; i++ ) {
			...
			for ( VFirstAttr ( list, & posn ); VAttrExists ( & posn ); VNextAttr ( & posn ) ) {
				...
				if ( VGetAttr ( VImageAttrList ( src ), "modality", NULL,
								VStringRepn, ( VPointer ) & buffer ) != VAttrFound ) continue;

				if ( strcmp( buffer, "conimg" ) != 0 ) continue;
			...
			if ( k == 0 ) VWarning( " no contrast map found in %s", in_filename );
		}
	...
	if ( strlen( report_filename ) > 1 ) fclose( fp );
