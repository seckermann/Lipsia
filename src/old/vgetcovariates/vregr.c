...
VglmGetRegr( VImageInfo *imageInfo, FILE *in_file, FILE *fp2, int *ptrvi, VString reportfile, VString regressor, char *voxelstr, char *extentstr, char *castr )
{
	...
	case 1: /* mm coord */
		...
		if ( !voxelstr ) VError( " attribute 'voxel' missing" );
	...
	case 2: /* Talairach */
		...
		if ( !extentstr || !strcmp( extentstr, "N" ) ) VError( " attribute 'extent' missing" );
		...
		if ( !castr || !strcmp( castr, "N" ) ) VError( " attribute 'ca' missing" );
	...
	case 1: /* 8-neighbours  (2D)  */
		...
		for ( r = y0 - 1; r <= y0 + 1; r++ ) {
			for ( c = x0 - 1; c <= x0 + 1; c++ ) {
				if ( r < 0 || r >= ptrvi[6] ) continue;

				if ( c < 0 || c >= ptrvi[7] ) continue;
	...
	for ( nobj1 = 0; nobj1 < ptrvi[4]; nobj1++ ) {
		if ( nobj1 < b0 || nobj1 > b1 ) continue;

		/* Check if an input file has a "zero-object" */
		sw = 0;
		nobj = nobj1 + ptrvi[3];

		if ( imageInfo[nobj].nbands < 2 ) sw = 1;
		...
		if ( sw == 0 ) {
			...
			if ( !ret ) VError ( "An error has occured while reading block data" );
			...
			for ( rr = 0; rr < rrws; rr++ ) {
				for ( cc = 0; cc < ptrvi[7]; cc++ ) {
					...
					if ( VPixel( mask, nobj1, rr + r0, cc, VBit ) == 0 ) continue;
					...
					if ( i != ptrvi[5] ) VWarning( "Some values of the regressor are less than the selected minval" );
	...
	/* Mean value */
	if ( relev_pixel == 0 ) VError( "There are no functional data for the selected voxel" );

	if ( relev_pixel > 1 ) {
		for ( i = 0; i < ptrvi[5]; i++ ) regr[i] /= ( double )relev_pixel;
	}
	...
	if ( regr1 ) {
		...
		if ( fp == NULL ) VError( " err opening %s", regressor );
		...
		if ( i != ptrvi[5] ) VError( "lengths of regressor and timesteps do not coincide" );

		if ( ptrvi[18] ) regr1 = VConvolveRegr( regr1, ptrvi );
	}
	...
	if ( ptrvi[14] == 1 ) {
		...
		if ( regr1 ) {
			...
			if ( i != regr1length ) VError( "This should not occur: i!=regr1length" );
	...
	if ( ptrvi[15] == 1 ) {
		if ( ptrvi[14] != 1 ) VError( "PPI requires normalization of regressors using '-norm true'" );

		if ( !regr1 ) VError( "PPI requires regressor using the option '-regr'" );

		if ( !regr2 ) VError( "regr2 is not defined" );

		for ( bb = 0; bb < ptrvi[5]; bb++ ) {
			regr2[bb] = regr[bb] * regr1[bb];
		}

		if ( ptrvi[16] == 1 ) regr2 = Orthogonalize( regr, regr1, regr2, ptrvi[5] );
	}

	/* Output */
	if ( fp2 == NULL ) VError( "error opening out file" );

	if ( ptrvi[1] == 0 ) {
		if ( ptrvi[17] < 10 || ptrvi[17] > 100000 ) VError( "TR invalid in input file" );
		...
		for ( bb = 0; bb < ptrvi[5]; bb++ ) {
			if ( regrnb > 0 ) VPixel( dest, 0, bb, 0, VFloat ) = ( VFloat )regr[bb];

			if ( regrnb > 1 ) VPixel( dest, 0, bb, 1, VFloat ) = ( VFloat )regr1[bb];

			if ( regrnb > 2 ) VPixel( dest, 0, bb, 2, VFloat ) = ( VFloat )regr2[bb];
		...
		if ( ! VWriteFile ( fp2, list ) ) exit ( 1 );
	} else {
		if ( ptrvi[1] == 1 ) {
			for ( bb = 0; bb < ptrvi[5]; bb++ ) {
				if ( regrnb == 1 ) fprintf( fp2, "%14.7f \n", regr[bb] );

				if ( regrnb == 2 ) fprintf( fp2, "%14.7f %14.7f \n", regr[bb], regr1[bb] );

				if ( regrnb == 3 ) fprintf( fp2, "%14.7f %14.7f %14.7f \n", regr[bb], regr1[bb], regr2[bb] );
			}
		} else {
			...
			for ( bb = 0; bb < ptrvi[5]; bb++ ) fprintf( fp2, "%1.7f ", regr[bb] );
			...
			if ( regr1 ) {
				...
				for ( bb = 0; bb < ptrvi[5]; bb++ ) fprintf( fp2, "%1.7f ", regr1[bb] );
				...
				if ( regr2 && ptrvi[15] == 1 ) {
					...
					for ( bb = 0; bb < ptrvi[5]; bb++ ) fprintf( fp2, "%1.7f ", regr2[bb] );
	...
	if ( strlen( reportfile ) > 2 ) {
		...
		if ( fp1 == NULL ) VError( "error opening report file %s", reportfile );
		...
		if ( ptrvi[10] == 0 ) fprintf( fp1, "# type        : %s\n", "single voxel" );

		if ( ptrvi[10] == 1 ) fprintf( fp1, "# type        : %s\n", "8adj" );

		if ( ptrvi[10] == 2 ) fprintf( fp1, "# type        : %s\n", "26adj" );
