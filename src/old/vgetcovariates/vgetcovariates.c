	...
	for ( nobj = 0; nobj < nobject; nobj++ ) {
		...
		if ( imageInfo[nobj].repn == VShortRepn ) {
			...
			if ( imageInfo[nobj].nbands > 1 ) {
				...
				if ( num_rows    != imageInfo[nobj].nrows )     VError( " number of rows is different" );

				if ( num_columns != imageInfo[nobj].ncolumns )  VError( " number of columns is different" );

				if ( num_bands   != imageInfo[nobj].nbands )    VError( " number of time steps is different" );

				if ( tr != trpre ) VError( " tr in objects is different" );

				if ( strcmp( voxelstr, imageInfo[nobj].voxel ) )   VError( " different voxel size in objects" );

				if ( strcmp( extentstr, imageInfo[nobj].extent ) ) VError( " different extent size in objects" );

				if ( strcmp( castr, imageInfo[nobj].ca ) )         VError( " different ca in objects" );
			}
		}
	}
