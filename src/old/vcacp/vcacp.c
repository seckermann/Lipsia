...
VImage VReSample3d(VImage src, VImage dest, int dest_nbands, int dest_nrows, int dest_ncolumns,
    ...
    if(detA == 0)
        VError(" transformation matrix matrix is singular");
    for(i = 0; i < 3; i++) {
        for(j = 0; j < 3; j++) {
            ainv[i][j] /= detA;
        }
    ...
    for(b = 0; b < src_nbands; b++) {
        for(r = 0; r < src_nrows; r++) {
            for(c = 0; c < src_ncols; c++) {
                ...
                rp += shift[1];
                cp += shift[2];
                if(bp <= -1.0 || bp >= src_nbands)
                    continue;
                if(rp <= -1.0 || rp >= src_nrows)
                    continue;
                if(cp <= -1.0 || cp >= src_ncols)
                    continue;
                bottom = VCeil(rp);
                top    = VFloor(rp);
                right  = VCeil(cp);
...
dpoint computePoint(dpoint point, ipoint fixpoint, double *angle) {
    ...
    if(detA == 0)
        VError(" rotation matrix is singular");
    for(i = 0; i < 3; i++) {
        for(j = 0; j < 3; j++) {
            ainv[i][j] /= detA;
        }
        ...
        for(z = 0 + d; z < cabands - d; z++) {
            for(y = 0 + d; y < carows - d; y++) {
                for(x = 0 + d; x < cacols - d; x++) {
                    if(rot == 0)
                        xx[i] = VPixel(ca_m, z, y, x, VUByte);
                    else
                        xx[i] = VPixel(ca_rot, z, y, x, VUByte);
                    i++;
                }
            }
        }
        ...
        for(z = 0 + d; z < cpbands - d; z++) {
            for(y = 0 + d; y < cprows - d; y++) {
                for(x = 0 + d; x < cpcols - d; x++) {
                    if(rot == 0)
                        xx[i] = VPixel(cp_m, z, y, x, VUByte);
                    else
                        xx[i] = VPixel(cp_rot, z, y, x, VUByte);
                    i++;
                }
            }
        }
                        ...
                        for(k = -1; k < 1; k++) {
                            ...
                            if(r >= dmin && r <= dmax) {
                                ...
                                if(VPixel(ca_mark, ca.z, ca.y, ca.x, VUByte) < 255) {
                                    ...
                                    rca = (double)VPixel(ca_corr, ca.z, ca.y, ca.x, VFloat);
                                if(VPixel(cp_mark, cp.z, cp.y, cp.x, VUByte) < 255) {
                                    ...
                                    rcp = (double)VPixel(cp_corr, cp.z, cp.y, cp.x, VFloat);
                            } else
                                continue;
                            if(rca > 0 && rcp > 0)
                                corr = rca * rcp;
    ...
    for(z = 0; z < dz; z++) {
        for(y = 0; y < N; y++) {
            for(x = 0; x < N; x++) {
                i = (int)(rotrows - N) / 2 + y;
                j = (int)(rotcols - N) / 2 + x;
                if(VPixel(rot, z, i, j, VUByte) > 1)
                    VPixel(norm, z, y, x, VUByte) = 255;
                else
                    VPixel(norm, z, y, x, VUByte) = 1;
            }
        }
    }
    return norm;
...
double MidSagittalPitch(VImage src, int N, int N2, int rg, int dz, ipoint mf) {
    ...
    for(b = 0; b < dz; b++) {
        for(c = 0; c < 256; c++) {
            for(r = 255; r >= 0; r--) {
                if((255 - r) <= (gsl_matrix_get(histo, b, c) / teiler[b]))
                    VPixel(histogramm, b, r, c, VUByte) = 0;
                else
                    VPixel(histogramm, b, r, c, VUByte) = 255;
            }
        }
    }
    ...
    for(b = 0; b < dz; b++) {
        for(r = 0; r < N; r++) {
            for(c = 0; c < N; c++) {
                ...
                if(grw > threshold[b])
                    VPixel(s_bin, b, r, c, VUByte) = 255;
                else
                    VPixel(s_bin, b, r, c, VUByte) = 1;
            }
        }
    }
    /* reflection at x=0 */
...
int main(int argc, char *argv[]) {
    ...
    if(CAtemplate == NULL)
        VError("CA template not found");
    if(CPtemplate == NULL)
        VError("CP template not found");
    if(VCtemplate == NULL)
        VError("VC template not found");
    ...
    for(VFirstAttr(list, &posn); VAttrExists(&posn); VNextAttr(&posn)) {
        if(VGetAttrRepn(&posn) == VImageRepn) {
            ...
            if(VPixelRepn(src) != VUByteRepn)
                VError("Input file must be ubyte");
            /* voxel */
