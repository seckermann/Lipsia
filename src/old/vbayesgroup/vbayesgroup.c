...
int main(int argc, char *argv[]) {
    ...
    */
    ...
    if(! f)
        VError("Failed to open input file %s", filename1);
    if(!(list1 = VReadFile(f, NULL)))
        exit(EXIT_FAILURE);
    fclose(f);
    ...
    if(mean1 == NULL || sigma1 == NULL)
        VError("Incorrect input image");
    /*
    ** read second input file
    */
    f = fopen((char *)filename2, "r");
    if(! f)
        VError("Failed to open input file %s", filename2);
    if(!(list2 = VReadFile(f, NULL)))
        exit(EXIT_FAILURE);
    fclose(f);
