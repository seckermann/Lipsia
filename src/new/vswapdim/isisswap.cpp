...
data::Image voxelSwapZ( data::Image src, unsigned int dim )
{
...
int main( int argc, char **argv )
{
	...
	app.init( argc, argv );
	std::list<data::Chunk> finChunkList;
	std::list<data::Image> finImageList;
	unsigned int dim = alongMap[app.parameters["along"].toString()];
	//go through every image
	BOOST_FOREACH( std::list<data::Image>::const_reference refImage, app.images ) {
		...
		if ( app.parameters["swap"].toString() == "image" || app.parameters["swap"].toString() == "both" ) {
			...
			case data::ValuePtr<int32_t>::staticID:
				...
				break;
			case data::ValuePtr<u_int64_t>::staticID:
