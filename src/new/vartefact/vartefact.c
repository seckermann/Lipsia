...
void
HighPassFilter(double *z,int n,float tr,double high)
{
  int i,nc;
...
void
VArtefact(VAttrList list,VFloat alpha,VFloat minval,VFloat highpass,VBoolean verbose)
{
  VImage src;
  VAttrListPosn posn;
  VString str;
  ...
  */
  ...
  for (VFirstAttr (list, & posn); VAttrExists (& posn); VNextAttr (& posn)) {
    ...
    if (slice%5 == 0) fprintf(stderr," slice= %5d\r",slice);
    slice++;
	...
	for (j=0; j<n; j++) z[j] = VPixel(src,j,r,c,VShort);
	HighPassFilter(z,n,tr,(double)highpass);
	...
	/*
	for (j=0; j<n; j++) {
	  fprintf(stderr," %d %f %d\n",j,z[j],(int)VPixel(src,j,r,c,VShort));
	}
	exit(0);
	*/
...
{
  static VFloat alpha  = 4;
  static VFloat minval = 0;
  static VFloat highpass = 100;
  ...
  static VOptionDescRec  options[] = {
    ...
    {"high",VFloatRepn,1,(VPointer) &highpass,VOptionalOpt,NULL,"Cutoff for highpass filter"},
