	...
	for ( b = 0; b < nslices; b++ ) {
		for ( r = 0; r < nrows; r++ ) {
			for ( c = 0; c < ncols; c++ ) {
				...
				for ( i = 0; i < n; i++ ) {
					if ( b >= VImageNBands( src[i] ) ) continue;

					if ( r >= VImageNRows( src[i] ) ) continue;

					if ( c >= VImageNColumns( src[i] ) ) continue;
				...
				case 9:  /* sum */
					...
					if ( u > VRepnMaxValue( VFloatRepn ) ) u = VRepnMaxValue( VFloatRepn ) - 1;
				...
				case 10:  /* coefficient of variation */
					...
					if ( u < 1.0e-6 ) continue;

					v = gsl_stats_sd( vec->data, vec->stride, n );

					if ( ABS( v ) < 1.0e-6 ) continue;
...
Usage:
	...
	if ( n >= NIMAGES ) VError( " too many images, max is %d", NIMAGES );


	/* read images  */
	for ( i = 0; i < n; i++ ) src1[i] = NULL;
	...
	for ( i = 0; i < n; i++ ) {
		...
		if ( src1[i] == NULL ) VError( " image %d not found", i );
	}
