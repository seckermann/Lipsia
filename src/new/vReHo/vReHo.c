...
VReHo(VAttrList list,VShort minval,VShort first,VShort length)
{
  ...
  */
  ...
  for (VFirstAttr (list, & posn); VAttrExists (& posn); VNextAttr (& posn)) {
    ...
    i++;
    if (i >= NSLICES) VError(" too many slices,max is %d",NSLICES);
  }
  nslices = i;
  ...
  */
  for (b=1; b<nslices-1; b++) {
    ...
    if (VImageNBands(src[b]) < 2) continue;
    for (r=1; r<nrows-1; r++) {
      for (c=1; c<ncols-1; c++) {
	...
	for (bb=b-1; bb<=b+1; bb++) {
	  if (VImageNBands(src[bb]) < 2) goto next;
	  for (rr=r-1; rr<=r+1; rr++) {
	    for (cc=c-1; cc<=c+1; cc++) {
	      if (VPixel(src[bb],0,rr,cc,VShort) < minval) goto next;
