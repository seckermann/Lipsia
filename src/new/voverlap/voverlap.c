...
Dist(VImage map1,VImage map2,int *lut)
{
  ...
  for (i=0; i<VImageNColumns(map1); i++) {
    ...
    if (j0 < 0) VError(" maps are inconsistent");
...
{ 
  ...
  ** read 1st file
  */
  if (! (list1 = VReadFile (in_file, NULL))) exit (1);
  fclose(in_file);
  ...
  if (map1 == NULL) VError(" map 1 not found");
  ...
  */
  ...
  if (map2 == NULL) VError(" map 2 not found");


  n1 = VImageNColumns(map1);
  n2 = VImageNColumns(map2);
  if (n1 != n2) VError(" n1 != n2");
  ...
  */
  ...
  for (VFirstAttr (list1, & posn); VAttrExists (& posn); VNextAttr (& posn)) {
    ...
    if (strncmp(VGetAttrName(&posn),"H",1) == 0) continue;
    if (strncmp(VGetAttrName(&posn),"W",1) == 0) continue;
