...
main( int argc, char *argv[] )
{
	...
	if ( low >= 0 )  f0 = 1.0 / low; /* schnell */

	if ( high >= 0 ) f1 = 1.0 / high; /* langsam */

	if ( type < 3 ) {
		fprintf( stderr, " frequency range:  [%.4f,%.4f]\n", f0, f1 );

		if ( low > high ) VError( " illegal freq range: %f %f", low, high );
	}
	...
	for ( VFirstAttr ( list, & posn ); VAttrExists ( & posn ); VNextAttr ( & posn ) ) {
		...
		if ( k < 1 ) VCopyImageAttrs ( src, dest );
		...
		for ( r = 0; r < nrows; r++ ) {
			for ( c = 0; c < ncols; c++ ) {
				...
				for ( j = 0; j < n; j++ ) in[j] = VPixel( src, j, r, c, VShort );
