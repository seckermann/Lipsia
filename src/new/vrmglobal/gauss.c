...
GaussMatrix( double sigma, gsl_matrix_float *S )
{
	...
	/* second level design matrix, no pre-coloring */
	if ( sigma < TINY ) {
	...
	for ( i = 0; i < m; i++ ) {
		...
		for ( j = 0; j < m; j++ ) {
			...
			x = gsl_vector_get( kernel, k );
			sum += x;
			gsl_matrix_float_set( S, i, j, x );
		}

		/* normalize */
		for ( j = 0; j < m; j++ ) {
			x = gsl_matrix_float_get( S, i, j );
			gsl_matrix_float_set( S, i, j, x / sum );
		}
	}
}
