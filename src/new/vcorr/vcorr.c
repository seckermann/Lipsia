...
main(int argc, char *argv[]) {
    ...
    nslices = k;
    if(start + len > n)
        VError(" illegal length %d, max= %d", start + len, n);
    if(len == 0)
        len = n;
    ...
    if(strlen(filename) > 1) {
        ...
        if(c < 0 || c >= ncols)
            VError(" illegal column address (%d), must be < %d", c, ncols);
        if(r < 0 || r >= nrows)
            VError(" illegal row address (%d), must be < %d", r, nrows);
        if(b < 0 || b >= nslices)
            VError(" illegal slice address (%d), must be < %d", b, nslices);
        ptr = array1->data;
