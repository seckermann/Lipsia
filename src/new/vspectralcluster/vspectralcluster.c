...
VSpectralClustering(VImage src,int nclusters,VShort ntype,VBoolean raster,VBoolean lapack)
{
  ...
  */
  for (i=0; i<nvectors; i++) {
    for (j=0; j<=i; j++) {
      ...
      if (u < 0) VSetPixel(src,0,i,j,0);
      if (v < 0) VSetPixel(src,0,j,i,0);
      if (ABS(u-v) > 0.0001) VError("matrix must be symmetric");
    }
  ...
  */
  ...
  if (!dest) VError(" err allocating output image, dims: %d %d %d\n",nslices,nrows,ncols);
  ...
  for (i=0; i<nvectors; i++) {
    ...
    if (b < 0 || b >= nslices) VError(" illegal slice coord %d",b);
    if (r < 0 || r >= nrows) VError(" illegal row coord %d",r);
    if (c < 0 || c >= ncols) VError(" illegal column coord %d",c);
...
main (int argc,char *argv[])
{
  ...
  if (nclusters < 2) VError(" number of clusters too small");
  ...
  for (VFirstAttr (list, & posn); VAttrExists (& posn); VNextAttr (& posn)) {
    ...
    if (VImageNRows(src) != VImageNColumns(src)) continue;
    if (VImageNBands(src) > 1) continue;
    if (VGetAttr(VImageAttrList(src),"map",NULL,VImageRepn,&map) != VAttrFound)
      continue;
    
    dest = VSpectralClustering(src,nclusters,ntype,raster,lapack);
    VSetAttrValue (& posn, NULL,VImageRepn,dest);
    if (raster) VAppendAttr(list,"cluster",NULL,VImageRepn,src);
    break;
  }
  if (dest == NULL) VError(" similarity matrix not found");
