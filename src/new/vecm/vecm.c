/*
** ECM - eigenvector centrality using linar correlations
**
** G.Lohmann, Mar 2009
*/
...
WriteOutput(VImage src, VImage map, int nslices, int nrows, int ncols, float *ev, int n) {
    ...
    for(i = 0; i < n; i++) {
        ...
        if(b < 0 || r < 0 || c < 0)
            VError("  WriteOutput: illegal address %d %d %d", b, r, c);
...
#pragma omp parallel for shared(progress) private(j) schedule(guided) firstprivate(mat,A)
    for(i = 0; i < n; i++) {
        ...
        for(j = 0; j <= i; j++) {
            ...
            if(k >= m)
                VError(" illegal addr k= %d, m= %d", k, m);
            A[k] = u;
        }
    }
