			...
			VFloat resolution, int ntimesteps, VShort minval, VBoolean compress, int *dst_nslices )
{
	...
	*/
	...
	if      ( strcmp( str1, "coronal" )  == 0 ) orient = CORONAL;
	else if ( strcmp( str1, "axial" )    == 0 ) orient = AXIAL;
	else if ( strcmp( str1, "sagittal" ) == 0 ) orient = SAGITTAL;
	else VError( "orientation must be axial or coronal" );
	...
	*/
	...
	for ( j = 0; j < nbands; j++ ) src_bands[j] = VAllBands;


	fd = open( in_filename, O_RDONLY );

	if ( fd == -1 ) VError ( "could not open infile %s", in_filename );

	for ( i = 0; i < ntimesteps; i++ ) {
		if ( i % 50 == 0 ) fprintf( stderr, "Processing %4d of %4d\r", i, ntimesteps - 1 );
		...
		for ( j = 0; j < nbands; j++ ) src_bands[j] = 0;
	...
	for ( j = 0; j < nbands; j++ ) VDestroyImage( image[j] );
