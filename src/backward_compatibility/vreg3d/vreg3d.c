...
int main(int argc, char *argv[]) {
    ...
    if(range != 0 && range != 1)
        VError(" illegal range (must be 'small' or 'big')");
    if(band1 > band2 && band2 >= 0)
        VError("<band1> must be less than <band2>");
    /* Read the input file: */
    list = VReadFile(in_file, NULL);
    if(! list)
        VError(" can't read input file");
    fclose(in_file);
    high_res = 99999;
    ...
    for(VFirstAttr(list, & posn); VAttrExists(& posn); VNextAttr(& posn)) {
        ...
        if(mdeft != NULL && epi_t1 != NULL)
            break;
    }
    if(mdeft == NULL)
        VError("Anatomical reference image not found");
    if(epi_t1 == NULL && refine == TRUE)
        VWarning(" no EPI-T1 image found");
    if(epi_t1 == NULL)
        refine = FALSE;
    if(band1 < 0)
        band1 = 0;
    ...
    */
    ...
    if(! list)
        VError("Error reading ref image");
    fclose(fp);
    for(VFirstAttr(list, & posn); VAttrExists(& posn); VNextAttr(& posn)) {
        if(VGetAttrRepn(& posn) != VImageRepn)
            continue;
        ...
        if(VPixelRepn(ref) != VUByteRepn)
            VError(" ref image must be of type ubyte");
        ...
        if(strcmp(str, "coronal") == 0) {
            ...
            if(refine)
                VRegRefine(ref_cor, epi_t1, transform, (int)band1, (int)band2);
            ...
            if(refine)
                VRegRefine(ref_sag, epi_t1, transform, (int)band1, (int)band2);
            VCopyImageAttrs(ref, transform);
            ...
            if(refine)
                VRegRefine(ref, epi_t1, transform, (int)band1, (int)band2);
            VCopyImageAttrs(ref, transform);
