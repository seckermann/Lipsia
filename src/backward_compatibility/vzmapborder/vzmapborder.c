...
Usage:
	...
	if ( ! list ) VError( "Error reading reference image" );
	...
	if ( ref == NULL ) VError( "reference image not found" );
	...
	for ( i = 0; i < in_files.number; i++ ) {
		...
		for ( VFirstAttr ( list, & posn ); VAttrExists ( & posn ); VNextAttr ( & posn ) ) {
			...
			if ( VPixelRepn( src ) != VFloatRepn ) continue; /* not a zmap */
...
PredispImage ( VImage src, VImage ref, VImage dest, VDouble pos, VDouble neg, VFloat zval )
{
	...
	if ( VPixelRepn( src ) != VFloatRepn ) VError( " image must be float" );

	if ( VPixelRepn( ref ) != VUByteRepn ) VError( " ref image must be ubyte" );
	...
	*/
	...
	if ( src0 == NULL ) float_pp = VImageData( src );

	if ( src0 != NULL ) float_pp = VImageData( src0 );

	for ( i = 0; i < npixels; i++ ) {
		if ( *float_pp >= pos || *float_pp <= neg ) *bin_pp = 1;
		else *bin_pp = 0;
